#version 300 es
/* CMYK halftone filter — riso-style 4-ink halftone with rotated screens,
 * spot inks (fluorescent pink M, federal blue C), per-ink
 * misregistration, paper grain on warm cream substrate.
 *
 * Each ink builds a rotated cell grid at its own screen angle, samples
 * the source at this fragment's cell-center (plus a few-px
 * misregistration offset), pulls the matching CMYK channel, and
 * returns a soft-edged dot whose radius scales with sqrt(coverage)
 * (equal-area dot growth — perceptually linear).
 *
 * Composite uses priority-mix rather than CMYK multiplicative blending
 * because riso spot inks aren't orthogonal RGB filters — dot adjacency
 * does most of the visual mixing in real prints. Standard CMYK screen
 * angles (Y=0, C=15, K=45, M=75 deg) prevent the four grids from
 * moiréing with each other. */
...
const float DOT_GAIN = 0.62;   // r = sqrt(cov) * GAIN. Above 0.5 dots
                               // can spill into neighbors but only the
                               // home cell is checked so they clip at
                               // the boundary — high-coverage areas
                               // look near-solid, visually fine.
...
void main() {
    ...
    // Paper grain — per-canvas-pixel darkening, holds substrate
    // texture even in flat-ink regions.
